...
const getAll = async (req, res) => {
  try {
    ...
    const result = await subCategoryModel.getAllCategories({
      page,
      limit,
      search
    })
...
const getById = async (req, res) => {
  const category = await subCategoryModel.getCategoryById(req.params.id)
  if (!category) return res.status(404).json({ message: 'Category not found' })
  res.json(category)
}

const getByIdPrivate = async (req, res) => {
  const category = await subCategoryModel.getCategoryByIdPrivate(req.params.id)
  if (!category) return res.status(404).json({ message: 'Category not found' })
  res.json(category)
}
...
const create = async (req, res, next) => {
  try {
    ...
    const category = await subCategoryModel.createCategory(name.trim())
...
const update = async (req, res, next) => {
  try {
    ...
    const category = await subCategoryModel.updateCategory(id, name.trim())
...
const remove = async (req, res, next) => {
  try {
    ...
    const categoryExists = await subCategoryModel.getCategoryById(id)
    if (!categoryExists) {
      throw new AppError('Không tìm thấy danh mục', 404)
    }

    const result = await subCategoryModel.deleteCategory(id)
